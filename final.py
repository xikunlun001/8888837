...
def LEM2(B, attriSet, data, decisionName):
    ...
    for s,b in enumerate(B,start=0):
        ...
        while G != []:
            ...
            #print T_rules_index[n]
            # end of while loop
        
            
        rules_Copy = copy.deepcopy(T_rules)
        rules_index_Copy = copy.deepcopy(T_rules_index)
        
        for n, i in enumerate(T_rules_index):
            
            if len(rules_Copy) <=1:
                break
            
            TempIndex = copy.deepcopy(rules_index_Copy)
            rules_index_Copy.remove(i)
            where_in_rules = TempIndex.index(i)
            
            newIndex = list(set.union(*map(set, rules_index_Copy)))
                
            if set(newIndex) == set(b):
                rules_Copy.remove(rules_Copy[where_in_rules])
            
            else:
                
                rules_index_Copy = TempIndex
                
        T_rules = rules_Copy
        T_rules_index = rules_index_Copy
        
        for n,i in enumerate(T_rules):
            print i
            #print T_rules_index[n]
...
# calculate A*
    ...
    LEM2(upperSet, attriA, data, decisionName)
